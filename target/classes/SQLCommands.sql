...
CREATE TABLE `Navigation`.`userpins` (
  ...
  `lat` INT NOT NULL COMMENT 'pins latitude position',
  `lon` INT NOT NULL COMMENT 'pins longitude position',
  `pinName` VARCHAR(45) NOT NULL COMMENT 'custom user name',
  PRIMARY KEY (`pinID`),
  UNIQUE INDEX `userID_UNIQUE` (`pinID` ASC)
)
...
CREATE TABLE `Navigation`.`preferences` (
  ...
  PRIMARY KEY (`userID`),
  UNIQUE INDEX `pinID_UNIQUE` (`userID` ASC)
  )
